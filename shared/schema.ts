...
// Mahila Samman Rashi through Punjab Gov (separate task - full form, category)
export const mahilaSammanPunjabSubmissions = pgTable("mahila_samman_punjab_submissions", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  appUserId: varchar("app_user_id").notNull().references(() => appUsers.id),
  villageId: varchar("village_id"),
  villageName: text("village_name"),
...
export const insertMahilaSammanPunjabSubmissionSchema = createInsertSchema(mahilaSammanPunjabSubmissions).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
