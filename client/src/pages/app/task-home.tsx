...
export default function TaskHome({ user, onLogout, onProfile }: TaskHomeProps) {
  ...
  const isMahilaSammanSakhi = user.registrationSource === "mahila_samman_sakhi";

  // For newly nominated Sakhis, show a very limited dashboard with only Mahila Samman Rashi task
  if (isMahilaSammanSakhi) {
    return (
              ...
              <div>
                ...
                {user.mappedAreaName && (
                  <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                    <Badge
                      variant="secondary"
                      className="bg-white/15 text-white/90 text-[10px] px-2 border-0 backdrop-blur-sm"
                    >
                      Sakhi
                    </Badge>
                    <span className="text-xs text-white/70" data-testid="text-user-area">
                      {user.mappedAreaName}
                    </span>
                  </div>
                )}
          ...
          <section>
            ...
            <div className="space-y-2.5">
              <Link href="/task/mahila-samman-rashi">
                <Card
                  className="group cursor-pointer bg-white border-slate-100 hover:border-purple-200 hover:shadow-md transition-all duration-200 border-2 border-purple-100"
                  data-testid="task-card-mahila-samman-sakhi"
                >
                  <CardContent className="p-4 flex items-center gap-3.5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        {language === "hi"
                          ? "यहां से महिलाओं की जानकारी जोड़ें और अपडेट करें"
                          : language === "pa"
                          ? "ਇਥੋਂ ਮਹਿਲਾਵਾਂ ਦੀ ਜਾਣਕਾਰੀ ਜੋੜੋ ਅਤੇ ਅੱਪਡੇਟ ਕਰੋ"
                          : "Use this to add and update Mahila Samman Rashi entries."}
                      </p>
                    </div>
                    <div className="flex-shrink-0">
                      <div className="w-8 h-8 rounded-full bg-purple-50 flex items-center justify-center group-hover:bg-purple-100 transition-colors">
