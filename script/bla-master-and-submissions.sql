...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'bla_submissions' AND column_name = 'religion_community') THEN
    ALTER TABLE bla_submissions ADD COLUMN religion_community TEXT;
  END IF;
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'bla_submissions' AND column_name = 'anniversary_date') THEN
    ALTER TABLE bla_submissions ADD COLUMN anniversary_date TEXT;
  END IF;
